...
- **Classification:** `category` (one), `scene_tags` (string[]), `custom_tags` (≤3 strings), optional `primary_language`, `talent` (string[]); legacy `event_type` / `community` / `genre` retained
...
- **`scene_tags`** — the scenes, cultures, sounds or formats the *event* is built around, zero or more ids: music (`afrobeats`, `amapiano`, `afro_house`, `highlife`, `dancehall`, `soca`, `rnb`, `hip_hop`, `gospel`, `alte`, `fuji`), culture (`nigerian`, `ghanaian`, `south_african`, `ethiopian`, `kenyan`, `caribbean`, `jamaican`, `trinidadian`, `pan_african`), format (`day_party`, `owambe`, `nollywood`, `documentary`, `standup`, `independence`, `homecoming`, `young_professionals`, `students`). They describe the event, never the buyer; nobody is labelled by what they bought.
- **`custom_tags`** — up to 3 organizer-authored strings (2–24 chars, Unicode letters/digits/space/& ' - .), kept as typed after whitespace cleanup; case-insensitive unique; may not collide with a canonical tag id or label. Searchable on Discover, never a filter, never auto-promoted to canonical.
- **`primary_language`** — optional; `null` means not specified (never assumed). Values unchanged.
- **`genre`** — **legacy, retained, no longer organizer-facing.** Not written on new events, never touched by edits; existing values stay on their documents. Reads still normalize it, and the migration fallback still derives scene tags from it.
