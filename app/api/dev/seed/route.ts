...
import { createPromoterLink, adjustPromoterStats } from "@/lib/promoters";
...
export async function GET(req: Request) {
  ...
  // ~12 days out at 8:00 PM Phoenix (UTC-7 → 03:00 UTC next day).
  const base = new Date(Date.now() + 12 * 86_400_000);
  const startsAt = Date.UTC(base.getUTCFullYear(), base.getUTCMonth(), base.getUTCDate(), 27, 0, 0);
  ...
  // A promoter link so the promoter persona has data.
  const tunde = await createPromoterLink({ eventId: event.id, organizerId: organizer.id, name: "Tunde", commissionCents: 200 });
  ...
  // 18 real orders: 14 General + 4 VIP, one ticket each; 6 checked in; first 5 via Tunde.
  let tickets = 0;
  let gross = 0;
  let checkedIn = 0;
  let tPromo = { orders: 0, tickets: 0, gross: 0 };
  ...
  for (let i = 0; i < 18; i++) {
    ...
    await orderRef.set({
      ...
      promoter_link_id: viaTunde ? tunde.id : null, promo_code_id: null,
    ...
    if (viaTunde) { tPromo = { orders: tPromo.orders + 1, tickets: tPromo.tickets + 1, gross: tPromo.gross + tier.price_cents }; }
  }

  await adjustPromoterStats(tunde.id, tPromo);
  ...
  return NextResponse.json({
    ...
    promoterTracker: `${site}/p/${event.id}/${tunde.code}`,
    note: "Tables unsold (try buying one). Early Bird sold out. 6 of 18 checked in. 5 orders via promoter Tunde.",
  });
}
